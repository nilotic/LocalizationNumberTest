...
//  weply
//
//  Created by Den Jo on 23/11/2018.
...
struct LocaleManager {
    ...
    static var language: Language? {
        guard let code = LanguageCode(rawValue: languageCode) else { return nil }
    ...
    static func set(language: Language?) -> Bool {
        guard let language = language, Bundle.set(language: language) == true, let data = try? JSONEncoder().encode(language) else { return false }
        
        UserDefaults.standard.set(data, forKey: UserDefaultsKey.languageCode)
    ...
    static func countryCode(from callingCode: String?) -> String? {
        guard let code = callingCode else { return nil }
    ...
    static func validate(language: Language?) -> Bool {
        guard let rawValue = language?.code.rawValue else { return false }
        return Bundle.main.path(forResource: rawValue, ofType: "lproj") != nil
    }
}
