...
final class Toast: NSObject {
    ...
    private func show(message: String?) {
        ...
        toastView.backgroundColor                           = UIColor(named: "grayscaleExtraDark") ?? #colorLiteral(red: 0.1882352941, green: 0.1960784314, blue: 0.2078431373, alpha: 1)
        toastView.alpha                                     = 0
    ...
    private func present(toastView: ToastView) {
        if appearedToasts.count == 0, containerView.superview == nil {
            ...
            containerView.bottomAnchor.constraint(equalTo: first.bottomAnchor, constant: 0).isActive                   = true
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.1, delay: 0.15, options: .curveEaseInOut, animations: {
                toastView.alpha     = 1.0
                toastView.transform = CGAffineTransform(scaleX: 0.98, y: 0.98)
            })
            
            UIView.animate(withDuration: 0.25, delay: 0.25, usingSpringWithDamping: 0.6, initialSpringVelocity: 1.0, options: .curveEaseInOut, animations: {
    ...
    private func dismissAll() {
        for toastView in appearedToasts.reversed() { // Prevent to Out of bounds exception.
    ...
    @objc private func didReceiveKeyboardFrameChanged(notification: Notification) {
        ...
        DispatchQueue.main.async {
            for toastView in self.appearedToasts {
                if let bottomConstraint = toastView.bottomConstraint {
